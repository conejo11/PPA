...
int multiplicarMPIblocos(matriz_bloco_t **mat_a, matriz_bloco_t **mat_b, matriz_bloco_t **mat_c, int **matFim, int divisor, int size){

	int rank;
	MPI_Comm_rank(MPI_COMM_WORLD, &rank);

	for(int i = rank; i < divisor; i += size){
		multiplicar_submatriz(mat_a[i], mat_b[i], mat_c[i]);
	}
